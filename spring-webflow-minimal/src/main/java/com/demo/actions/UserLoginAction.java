...
import com.demo.bean.LoginInfo;
...
public class UserLoginAction implements Serializable{
	...
	public boolean validateUserEntry(LoginInfo loginInfo, MessageContext messageContext) {
		...
		if(!loginInfo.getUserId().equalsIgnoreCase("manoj") && !loginInfo.getPassword().equals("12345")) {
			messageContext.addMessage(
	...
	public boolean validateUserId(LoginInfo loginInfo,  MessageContext messageContext) {
		...
		if(loginInfo.getUserId().equalsIgnoreCase("manoj")) {
			//manoj is NOT in the database, so returning false
			return false;
		}
		//userId is existing
		return true;
